...
**Escopo desta versão** (limites deliberados, não bugs — ver "Limitações conhecidas"): **GET/POST/PUT/PATCH/DELETE/HEAD** (nativos) e **OPTIONS** (via `fetch(..., method="OPTIONS")`) geram teste real — outros métodos (ex.: TRACE, CONNECT) caem no fallback; autenticação suportada é **Bearer Token/API Key/Basic Auth**, e só quando o valor é uma referência pura a uma variável (`{{nome}}`, nunca um literal fixo nem herdado de Collection/pasta); body suportado é **JSON (raw) ou multipart/form-data**, presente em qualquer um dos métodos acima (a presença de body é decidida pelo request normalizado, não pelo verbo).
...
- **Schema inferido a partir de exemplo(s), não de contrato formal** — os
  ...
  mas não o elimina.
- **Playwright (`--target playwright`/`all`) gera testes reais para
  GET/POST/PUT/PATCH/DELETE/HEAD/OPTIONS** — métodos fora dessa whitelist
  (ex.: TRACE, CONNECT) caem no fallback (`@pytest.mark.skip` + warning
  `HTTP_METHOD_NOT_SUPPORTED`), sem gerar nenhuma asserção enganosa.
  Autenticação herdada da Collection/pasta ("Inherit auth from parent") não
  é resolvida (tratada como "sem autenticação"); só Bearer Token/API
  Key/Basic Auth declarados diretamente na request, com o valor sendo uma
  referência pura a `{{variável}}`, são suportados.
