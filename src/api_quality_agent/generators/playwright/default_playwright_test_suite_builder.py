...
def _render_conftest(endpoint_tests: Sequence[GeneratedEndpointTest]) -> str:
    ...
    return (
        ...
        "        # P2.1 (evidência HTTP): `params={...}` passado explicitamente\n"
        "        # no call site (ver playwright_endpoint_test_generator.py) —\n"
        "        # nunca reparseado a partir da URL final devolvida pelo\n"
        "        # Playwright, que já inclui a query string por conta própria.\n"
        "        \"query_parameters\": _json_safe(query_parameters) or {},\n"
